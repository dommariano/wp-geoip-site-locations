...
/**
 * Retrieved the visitor IP address.
 */
...
/**
 * If the visitor has vesited the site more than once, determine the site
 ...
 */
if ( in_array( $ip, array( '', GEOIPSL_RESERVED_IP, GEOIPSL_INVALID_IP ) ) ) {
 ...
 */
if ( $proxy_score && 'on' !=  $geoipsl_settings->get( 'query_proxies_status' ) ) {
  wp_redirect( add_query_arg( array( 'welcome' => 'home' ), get_home_url() ) );
  exit;
}
 ...
 */
if ( '' == $tracking_info && 'on' == $geoipsl_settings->get( 'use_geoip_detection' ) ) {
  ...
  $blog_ids = array( intval( $tracking_info ) );
}
